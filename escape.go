...
func unescapeBytes(in, out []byte) ([]byte, error) {
	firstBackslash := bytes.IndexByte(in, '\\')
	if firstBackslash == -1 {
		return in, nil
	}

	// Get a buffer of sufficient name (allocate if needed)
	if cap(out) < len(in) {
	...
	for len(in) > 0 {
		// unescapeBytes the next escaped character
		inLen, bufLen := unescapeToUTF8(in, buf)
		if inLen == -1 {
			return nil, errors.New("malformed unescape")
		}
